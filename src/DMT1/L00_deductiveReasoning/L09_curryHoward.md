...
A predicate is just a parameterized
proposition. Think of a predicate as
a proposition with placeholders that
you fill in by applying the predicate
to appropriate arguments. Let's see a
concrete example.

As we'll now see, we use predicates
to represent *properties* of things.
The things are the arguments. When you
apply a predicate to them, you get a
proposition. If the proposition is
valid, then we say the object has the
property that the predicate encodes.
...
--example : isEven 5 := rfl
```

#### Relations

A relation connects objects of different
(or the same) types. We represent relations
as predicate with multiple arguments.

```lean
inductive Likes : Person → Person → Prop where
| mlm : Likes Mary Mary
| klk : Likes Kevin Kevin
| blb : Likes Bill Bill
...
def reflexive {α : Type} : (α → α → Prop) → Prop :=
  fun r =>
    ∀ (a : α), r a a

example : reflexive Likes := _
