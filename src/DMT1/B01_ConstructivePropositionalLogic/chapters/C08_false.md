...
-- IMPOSSIBLE: Cannot prove True → False (no way to produce False from True)
-- Uncomment the following line to see Lean's error showing the stuck proof state:
...
-- IMPOSSIBLE: Cannot create a function Unit → Empty (no Empty values exist)
-- Uncomment the following line to see Lean's error showing the stuck proof state:
