...
a collection of expression-composing operators that take
...
syntax of a formal language defines only the set of
...
Expr type. The available constructors describe all
...
var_[3], for example, to mean (var.mk 3). It's not a lot
of help, but it's a start.
...
We do need to let Lean know the result should be a "var".
```lean
...
- here, op can be and, or, implies, iff, and so forth

We call the only unary operator *not*, and the usual binary operators
by their usual names. Remember, these are meaningless symbols in the formal
*syntax* of propositional logic. We give them names that will reflect their
usual meanings when we define *semantics* for syntactic expressions.
...
this point! We encourage you to follow it carefully
to get the gist.
...
syntax for propositional logic. Note that some definitions
