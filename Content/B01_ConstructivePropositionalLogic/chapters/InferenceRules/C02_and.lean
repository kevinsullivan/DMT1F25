...
theorem proofAndCommutes : andCommutes :=
  fun (P Q : Prop) =>               -- assume propositions, P, Q
    And.intro                       -- construct proof of conj
      ...
      -- EXERCISE: Prove right conjunct (Q ∧ P → P ∧ Q)
      (
...
Doing this makes them bound throughout the rest of the
...
theorem proofAndAssoc : P ∧ (Q ∧ R) ↔ (P ∧ Q) ∧ R :=
  ...
  (
    ...
    by (
      ...
      let q := h.right.left
      let r := h.right.right
      let pq := And.intro p q   -- assemble and return
      exact (And.intro pq r)    -- the final proof object
    )
  )
  -- EXERCISE: Prove reverse direction: (P ∧ Q) ∧ R → P ∧ Q ∧ R
  (
    fun
    (h : (P ∧ Q) ∧ R) =>
    (
