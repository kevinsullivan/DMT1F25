...
`+`) has a constructor for the `n + 0` case — it's essentially
an axiom that `n + 0 = n`. The definition provides no such axiom
for `0 + n`, so that equality must be proved by induction.
@@@ -/
...
-- Substitution to prove n + 1 = m + 1 from n = m
